...
func (s *SMTPSender) SendBuddyAccepted(_ context.Context, p BuddyAcceptedParams) error {
	msg := s.buildBuddyAcceptedMessage(p)
	addr := fmt.Sprintf("%s:%d", s.cfg.Host, s.cfg.Port)
	var auth smtp.Auth
	if s.cfg.Username != "" {
		auth = smtp.PlainAuth("", s.cfg.Username, s.cfg.Password, s.cfg.Host)
...
func buildBuddyAcceptedHTML(p BuddyAcceptedParams) string {
	buddyName := html.EscapeString(p.BuddyName)
	goalTitle := html.EscapeString(p.GoalTitle)
	dashURL := html.EscapeString(p.DashboardURL)
...
</html>`, buddyName, goalTitle, dashURL)
}
