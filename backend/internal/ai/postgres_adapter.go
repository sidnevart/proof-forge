...
// PostgresProofReader implements ProofReader for growth dossier generation.
type PostgresProofReader struct {
	pool *pgxpool.Pool
}

func NewPostgresProofReader(pool *pgxpool.Pool) *PostgresProofReader {
	return &PostgresProofReader{pool: pool}
}

// GetProofTextsForDossier collects all approved check-in texts grouped by goal for the period.
func (r *PostgresProofReader) GetProofTextsForDossier(ctx context.Context, userID int64, startDate, endDate string) ([]DossierInput, error) {
	rows, err := r.pool.Query(ctx, `
		SELECT g.title, e.text_content, $3::text AS start_date, $4::text AS end_date
		FROM check_ins ci
		JOIN goals g ON g.id = ci.goal_id
		LEFT JOIN evidence_items e ON e.check_in_id = ci.id AND e.kind = 'text'
		WHERE ci.user_id = $1
		  AND ci.status = 'approved'
		  AND ci.approved_at >= $3::date
		  AND ci.approved_at <= $4::date
		ORDER BY g.id, ci.approved_at
	...
	byGoal := make(map[string]*DossierInput)
	var order []string
	for rows.Next() {
		var title, text, sd, ed string
		if err := rows.Scan(&title, &text, &sd, &ed); err != nil {
			continue
		}
		if _, exists := byGoal[title]; !exists {
			byGoal[title] = &DossierInput{GoalTitle: title, StartDate: sd, EndDate: ed}
			order = append(order, title)
		}
		if text != "" {
			byGoal[title].ProofsTexts = append(byGoal[title].ProofsTexts, text)
		}
	}

	result := make([]DossierInput, 0, len(order))
	for _, title := range order {
		result = append(result, *byGoal[title])
	}
	return result, nil
}
